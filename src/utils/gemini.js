...
export function initGemini(apiKey) {
  if (process.env.LLM_PROVIDER === "ollama") {
    console.log("ℹ️ Using Ollama local provider. No Gemini API key needed.");
    return null;
  }
  if (!apiKey) {
...
export async function callGemini({
  ...
  const client = isOllama ? null : getClient();
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (isOllama) {
        ...
        rawText = resJson.response || "";
        inputTokens = resJson.prompt_eval_count || Math.ceil(fullPrompt.length / 4);
        outputTokens = resJson.eval_count || Math.ceil(rawText.length / 4);
